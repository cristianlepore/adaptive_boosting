...
calculate_error <- function(w, data, y, k) {
  numElem <- length(y)
  error <- 0
  gap2 <- rep(0, numElem)
...
K_folds <- function(k, data) {
  ...
  for(i in 1:1) {
    ...
    w <- perceptron(training, y_training)
    
    # Errore sul singolo validation set.
    er <- er + calculate_error(w, validation, y_validation, k)
  }
...
trainingset <- as.matrix(trainingset[,-10])
...
# Cross-Validazione.
# w <- K_folds(k, trainingset)

# Faccio girare il percettrone.
list <- perceptron(trainingset, y_train)

# Costruisco il grafo.
result <- create_graph(list$gap)

# Verifico sul testset.
gap2 <- calculate_error(list$model, testset[,-10], y_test, k)
...
barplot(counts,
        ...
        ylab = "Istanze",
        main = "Istogramma dell'errore - 20 epoche"
)
