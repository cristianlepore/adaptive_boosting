...
K_folds <- function(k, data, depth) {
  ...
  for(i in 1:k) {
    ...
    adaBoost <- AdaBoost(training, numberOfRow_training, y_training, depth)
...
for(depth in 1:10) {   #Utilizzo solo due layer di profondità dell'albero.
  # Cross-Validazione.
  ...
  adaBoost <- AdaBoost(as.matrix(trainingset[,-10]), nrow(trainingset), y_train, depth)
  # Ritorno l'etichetta degli elementi del testset secondo l'albero di decisione.
