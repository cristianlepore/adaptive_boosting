...
gain <- function(data, datasetEntropy, w, y) {
  ...
  for(colNumber in 1:ncol(data)) {
    ...
    for (sepN in 1:(N-1)) {
      # Faccio un controllo se la lunghezza della foglia è zero allora esco.
...
column_stump <- function(data, datasetEntropy, w, y, depth, colNumber) {
  ...
  if(length(unique(y))==1 || depth == 0) {
    ...
    partition <- gain(data, datasetEntropy, w, y)
...
boost <- function(data, datasetEntropy, w, y, depth, T) {
  ...
  for(t in 1:T) {
    ...
    stump <- column_stump(data, datasetEntropy, w, y, depth, colNumber)
...
AdaBoost <- function(data, numberOfRow, y, depth, T) {
  ...
  # Learner.
  adaBoost <- boost(data, datasetEntropy, w, y, depth, T)
